// astro-devbar — the Astro integration. Dev only by construction: outside `astro dev`
...
import { devbarMiddleware } from './server/middleware.mjs';

const PKG = fileURLToPath(new URL('..', import.meta.url));

/** @param {import('../types.d.ts').DevbarOptions} [options] */
export default function devbar(options = {}) {
  let routes = [];
  return {
    name: 'astro-devbar',
    hooks: {
      'astro:config:setup': ({ command, config, injectScript, updateConfig }) => {
        ...
        injectScript('page', 'import "astro-devbar/client";');
        updateConfig({
          vite: {
            ...
            optimizeDeps: { exclude: ['astro-devbar'] },
            ...
            plugins: [{
              ...
              configureServer(server) {
                server.middlewares.use(devbarMiddleware({ root, pkg: PKG, options, routes: () => routes }));
              },
            }],
          },
        });
